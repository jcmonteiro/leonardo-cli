...
func (s *GenerationService) Download(id, outputDir string) (domain.DownloadResult, error) {
	...
	for i, imgURL := range status.Images {
		...
		if err := writeSidecarFile(destPath, id, i+1, imgURL, status.Raw); err != nil {
			return domain.DownloadResult{}, fmt.Errorf("writing sidecar for image %d: %w", i+1, err)
		}
...
func writeSidecarFile(imagePath, generationID string, imageIndex int, imageURL string, raw []byte) error {
	...
	if err := json.Unmarshal(raw, &decoded); err == nil {
		sidecar["generation"] = decoded
		if generation, ok := decoded["generations_by_pk"].(map[string]interface{}); ok {
			sidecar["parameters"] = generation
		}
	} else if len(raw) > 0 {
		sidecar["raw"] = string(raw)
	}
